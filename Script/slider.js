class ISlider {
    ...
    static createInstances() {
        document.querySelectorAll('[data-slider="i-slider"]').forEach((el) => {
            ...
            this.contains.push({ el, slider: new ISlider(el, params) });
    ...
    constructor(selector, config) {
        ...
        this._elWrapper = this._el.querySelector(ISlider.SEL_WRAPPER);
        this._elItems = this._el.querySelector(ISlider.SEL_ITEMS);
        this._elsItem = this._el.querySelectorAll(ISlider.SEL_ITEM);
        this._elsIndicator = this._el.querySelectorAll(ISlider.SEL_INDICATOR);
        this._btnPrev = this._el.querySelector(ISlider.SEL_PREV);
        this._btnNext = this._el.querySelector(ISlider.SEL_NEXT);
    ...
    _addEventListener() {
        this._el.addEventListener('click', (e) => {
            this._autoplay('stop');
            if (e.target.classList.contains(ISlider.CLASS_CONTROL)) {
                e.preventDefault();
        ...
        if (this._config.loop) {
            this._elItems.addEventListener('islider-start', () => {
    ...
    _changeActiveItems() {
        this._stateItems.forEach((item, index) => {
            if (item) {
                this._elsItem[index].classList.add(ISlider.CLASS_ITEM_ACTIVE);
            } else {
                this._elsItem[index].classList.remove(ISlider.CLASS_ITEM_ACTIVE);
            }
            if (this._elsIndicator.length && item) {
                this._elsIndicator[index].classList.add(ISlider.CLASS_INDICATOR_ACTIVE);
            } else if (this._elsIndicator.length && !item) {
                this._elsIndicator[index].classList.remove(ISlider.CLASS_INDICATOR_ACTIVE);
            }
        });
    }
    ...
    _move() {
        ...
        if (!this._config.loop) {
            ...
            if (this._btnPrev) {
                this._btnPrev.classList.remove(ISlider.CLASS_CONTROL_HIDE);
                this._btnNext.classList.remove(ISlider.CLASS_CONTROL_HIDE);
            }
            if (this._btnPrev && transform === -limit) {
                this._btnNext.classList.add(ISlider.CLASS_CONTROL_HIDE);
            } else if (this._btnPrev && transform === 0) {
                this._btnPrev.classList.add(ISlider.CLASS_CONTROL_HIDE);
            }
        }
        if (this._direction === 'next') {
        ...
        this._elItems.dispatchEvent(new CustomEvent('islider-start', {
            bubbles: true
        }));
    }
    ...
    _reset() {
        ...
        this._autoplay('stop');
        this._elItems.classList.add(ISlider.TRANSITION_OFF);
        this._elItems.style.transform = 'translate3D(0px, 0px, 0.1px)';
        this._setInitialValues();
        window.requestAnimationFrame(() => {
            this._elItems.classList.remove(ISlider.TRANSITION_OFF);
        });
    }
